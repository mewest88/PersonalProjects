...
std::vector<Command> getCommands(const std::vector<std::string>& tokens){
  ...
  for(int i = 0; i < ret.size(); ++i){
	...
	ret[i].fdStdin = 0; //0 is read, 1 is write
	ret[i].fdStdout = 1;
	...
	for(int j = first + 1; j < last; ++j){
	  if(tokens[j] == ">" || tokens[j] == "<" ){
		//I/O redirection
          if(tokens[j] == ">") {
              ret[i].fdStdout = open(tokens[j+1].c_str(), O_WRONLY);
          } else {
	...
	if(i > 0){
	  //there are multiple commands.  Open a pipe andConnect the ends to the fds for the commands!
        int fd[2];
        pipe(fd);
